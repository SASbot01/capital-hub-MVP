...
export default function CompanyJobsPage() {
  ...
  const [callTool, setCallTool] = useState("CALENDLY");
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)] gap-6">
        ...
        <div className="bg-white rounded-3xl border border-neutral-200 px-6 py-5 shadow-sm">
          ...
          <form className="space-y-3" onSubmit={handleCreateJob}>
            ...
            <div>
              <label className="block text-[11px] text-neutral-500 mb-1">
                Herramienta para entrevistas
              </label>
              <div className="grid grid-cols-3 gap-2 text-[11px]">
                {["CALENDLY", "ZOOM", "WHATSAPP"].map((tool) => (
                  <button
                    key={tool}
                    type="button"
                    onClick={() => setCallTool(tool)}
                  ...
                  >
                    {tool === "CALENDLY" ? "Calendly" : tool === "ZOOM" ? "Zoom" : "WhatsApp"}
                  </button>
                ))}
              </div>
              <input
